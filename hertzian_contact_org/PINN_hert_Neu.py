...
import os,sys
sys.path.append("")
from dem_hyperelasticity.hertzian_contact_org import define_structure_Neu as des
from dem_hyperelasticity.MultiLayerNet import *
from dem_hyperelasticity.hertzian_contact_org import PINNmodel_neu as md
from dem_hyperelasticity import Utility as util
from dem_hyperelasticity.hertzian_contact_org import config as cf
from dem_hyperelasticity.IntegrationLoss import *
from dem_hyperelasticity.hertzian_contact_org.PINNmodel_neu import *
from dem_hyperelasticity.hertzian_contact_org import pin_alg as pin
import numpy as np
import time
import torch
import sys
...
class DeepEnergyMethod:
    ...
    def train_model(self, data, neumannBC, dirichletBC, LHD, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                dir_loss_1 = bc_u_crit[0][0]
                dir_loss_2 = bc_u_crit[1][1]
                neu_loss_1 = bc_t_crit[0][1]
                neu_loss_2 = torch.sum(bc_t_crit[1])
                neu_loss_3= torch.sum(bc_t_crit[2])
                neu_loss_4 = bc_t_crit[3][0]
                neu_loss = neu_loss_1+neu_loss_2+neu_loss_3
                # storedEnergy = 
                energy_loss = strong  # storedEnergy get problem
                loss = strong+ss+100*dir_loss_1+100*dir_loss_2+0.1*neu_loss_1+0.1*neu_loss_2+0.1*neu_loss_3+0.1*neu_loss_4# u is relate to bc_t #normalization
    ...
    def evaluate_model(self, x, y, z):
        ...
        Eyy = np.abs(s_pred[:, 1]+0.1).reshape(Ny, Nx,1)
        Exy = np.abs(s_pred[:, 2]).reshape(Ny, Nx,1)
...
if __name__ == '__main__':
    ...
    shape = [cf.Nx, cf.Ny]
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 100 #550 exp

    cf.filename_out = "dem_hyperelasticity/hertzian_contact_org/output/org_us"
