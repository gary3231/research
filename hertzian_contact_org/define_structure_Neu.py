...
from dem_hyperelasticity.hertzian_contact_pin.config import *
...
def setup_domain():
    ...
    bcl_u_2 = np.ones(np.shape(bcl_u_pts)) *[0, 0]
    # bcl_u_2[:,0] = 0.0293*bcl_u_pts_2[:,0]
    # bcl_u_2[:,1] = 0.0684*bcl_u_pts_2[:,1]
    ...
    A = (1/Nx)*(1/Ny)
    r = np.sqrt(A/np.pi)
    p2 = B/(np.pi*r**2)*((np.pi*2*r)**2)
    # g = -((1-r)-(1-0.0684))-r
    bcr_t_2[:,1] = -0.0684*p2*0.1
    ...
    s=np.hstack((bcl_u_pts_idx,bcl_u_pts_idx_2,bcr_t_pts_idx_3,bcr_t_pts_idx))
