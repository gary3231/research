...
from dem_hyperelasticity.DEM_contact.config import *
...
def setup_domain():
    ...
    c = 0
    xGrid, yGrid = np.meshgrid(lin_x, lin_y)
    x1D = xGrid.flatten()
    y1D = yGrid.flatten()
    dom = np.concatenate((np.array([x1D]).T, np.array([y1D]).T), axis=-1)
    print(dom.shape)
    ...
    bcr_t_pts_idx = np.where((dom[:, 1] == Height))
    bcr_t_pts_idx_2 = np.where((dom[:, 0] == Length) & ((dom[:, 1] > 0.8) | (dom[:, 1] < 0.6)))
    bcr_t_pts_idx_3 = np.where((dom[:, 0] == Length) & ((dom[:, 1] <= 0.8) & (dom[:, 1] >= 0.6)))
    ...
    bcr_t_3 = np.ones(np.shape(bcr_t_pts_3)) * [5, 0]
    ...
    ax.scatter(bcr_t_pts_3[:, 0], bcr_t_pts_3[:, 1], s=0.5, facecolor='red')
    ...
    boundary_neumann = {
        ...
        "neumann_4": {
            "coord": bcr_t_pts_4, #size is same as below
            "known_value": bcr_t_4,   #size is [N,2]
            "penalty": bc_right_penalty,  #in config file
            "idx":bcr_t_pts_idx_4
        }
    ...
    # s=np.hstack((bcl_u_pts_idx,bcr_t_pts_idx_2,bcr_t_pts_idx_3,bcr_t_pts_idx))
