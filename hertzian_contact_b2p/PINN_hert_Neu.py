    ...
    def train_model(self, data, neumannBC, dirichletBC, LHD, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                loss = ss+strong+1000*dir_loss+0.1*neu_loss_1+0.1*neu_loss_2+0.1*neu_loss_3+1300*lkkt+0.1*lkkt_t# 0.5 best
                # loss = loss*1000
                optimizer.zero_grad()
                loss.backward()
...
if __name__ == '__main__':
    ...
    shape = [cf.Nx, cf.Ny]
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 800 #550 exp

    # cf.filename_out = "dem_hyperelasticity/hertzian_contact_square/output/hert_20_itr_sb"
    cf.filename_out = "dem_hyperelasticity/hertzian_contact_b2p/output/hert_fb_not_symmetric_1300"
    ...
    cf.filename_out = "dem_hyperelasticity/hertzian_contact_b2p/output/hert_fb_not_symmetric"
    dem.train_model(dom, boundary_neumann, boundary_dirichlet, [cf.Length, cf.Height, cf.Depth], cf.iteration, cf.lr)
    end_time = time.time() - start_time
    print("End time: %.5f" % end_time)
    z = np.array([0.])
