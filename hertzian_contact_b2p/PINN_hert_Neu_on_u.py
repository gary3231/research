...
import os,sys
sys.path.append("")
from dem_hyperelasticity.hertzian_contact_square import define_structure_Neu as des
from dem_hyperelasticity.MultiLayerNet import *
from dem_hyperelasticity.hertzian_contact_square import PINNmodel_neu as md
from dem_hyperelasticity import Utility as util
from dem_hyperelasticity.hertzian_contact_square import config as cf
from dem_hyperelasticity.IntegrationLoss import *
from dem_hyperelasticity.hertzian_contact_square.PINNmodel_neu import *
from dem_hyperelasticity.hertzian_contact_square import pin_alg as pin
import numpy as np
import time
import torch
    ...
    def train_model(self, data, neumannBC, dirichletBC, LHD, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                u_pred.double()
                # s_pred.double()
                strong, _ = self.energy.getStoredEnergy(u_pred, x,-1)
                ...
                for i, vali in enumerate(neuBC_coordinates):
                    ...
                    #             neuBC_values[i][j][1]=torch.add(val[i][j,1],torch.from_numpy(pf).float().to(dev))
                            ...
                            # neuBC_values[i][j+1][1]=torch.add(val[i][j+1,1],torch.from_numpy(pf/2).float().to(dev))
                    _,neu = self.energy.getStoredEnergy(neu_u_pred, neuBC_coordinates[i],i)
                    if i!=3:
                ...
                loss = strong+100*dir_loss+0.1*neu_loss_1+0.1*neu_loss_2+0.1*neu_loss_3+10*lkkt_u+100*lkkt_n+10*lkkt_un+0.1*lkkt_t # u is relate to bc_t #normalization
                # loss = strong+100*dir_loss+0.1*neu_loss_1+0.1*neu_loss_2+0.1*neu_loss_3+100*lkkt+0.1*lkkt_t
                # loss = loss*1000
                optimizer.zero_grad()
                loss.backward()
                print('Iter: %d Loss: %.6e strong_form: %.6e dir_bc: %.6e neu_bc: %.6e  Time: %.3e '
                      % (t + 1, loss.item(), strong.item(),lkkt_u.item(),lkkt_n.item(), time.time() - it_time))
...
if __name__ == '__main__':
    ...
    # cf.filename_out = "dem_hyperelasticity/hertzian_contact_square/output/hert_20itr_onlyu_sb_soft"
