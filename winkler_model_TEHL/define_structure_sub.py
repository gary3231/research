...
from dem_hyperelasticity.winkler_model_TEHL.config_sub import *
...
def setup_domain():
    ...
    bcl_u_pts_idx_1 = np.where((dom[:,1] == (y_min+Height)))
    bcl_u_pts_idx_2 = np.where(dom[:, 0] == x_min)
    bcl_u_pts_idx_3 = np.where(dom[:,1] == (x_min+Length))
    ...
    # ax.scatter(bcl_u_pts_3[:, 0], bcl_u_pts_3[:, 1], s=0.5, facecolor='red')
    # ax.scatter(bcr_t_pts[:, 0], bcr_t_pts[:, 1], s=0.5, facecolor='green')
    # ax.scatter(bcr_t_pts_1[:, 0], bcr_t_pts_1[:, 1], s=0.5, facecolor='green')
    ...
    boundary_dirichlet = {          #two dict
        ...
        "dirichlet_3": {
            "coord": bcl_u_pts_2,
            "known_value": np.expand_dims(bcl_u_2[:,0], axis = 1),
            "penalty": bc_left_penalty
        },
        "dirichlet_4": {
            "coord": bcl_u_pts_3,
            "known_value": np.expand_dims(bcl_u_3[:,0], axis = 1),
            "penalty": bc_left_penalty
        },
        # adding more boundary condition here ...
    }
