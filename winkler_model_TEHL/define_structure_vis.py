...
from dem_hyperelasticity.winkler_model_TEHL.config import *
...
def setup_domain():
    ...
    bcl_u_pts_idx = np.where((dom[:,0]**2+dom[:,1]**2) > ((1.5*a)**2))
    bcl_u_pts_idx_1 = np.where((dom[:,0] == 0) & (dom[:,1] == 0)  )
    # bcl_u_pts_idx_2 = np.where(dom[:,1] == y_min)
    # bcl_u_pts_idx_3 = np.where(dom[:,1] == y_min)
    ...
    # ax.scatter(bcr_t_pts[:, 0], bcr_t_pts[:, 1], s=0.5, facecolor='red')
    ...
    boundary_neumann = {
        ...
        "neumann_1": {
            ...
            "known_value": np.expand_dims(bcr_t[:,0], axis = 1),   #size is [N,2]
            "penalty": bc_right_penalty,  #in config file
            "idx":bcr_t_pts_idx
        },
        "neumann_2": {
            "coord": bcr_t_pts_1, #size is same as below
            "known_value": np.expand_dims(bcr_t_1[:,0], axis = 1),   #size is [N,2]
            "penalty": bc_right_penalty,  #in config file
            "idx":bcr_t_pts_idx_1
        },
        "neumann_3": {
            "coord": bcr_t_pts_2, #size is same as below
            "known_value": np.expand_dims(bcr_t_2[:,0], axis = 1),  #size is [N,2]
            "penalty": bc_right_penalty,  #in config file
            "idx":bcr_t_pts_idx_2
        },
        "neumann_4": {
            "coord": bcr_t_pts_3, #size is same as below
            "known_value": np.expand_dims(bcr_t_3[:,0], axis = 1),   #size is [N,2]
            "penalty": bc_right_penalty,  #in config file
            "idx":bcr_t_pts_idx_3
        }
    ...
    boundary_dirichlet = {          #two dict
        ...
        "dirichlet_1": {
            "coord": bcl_u_pts,
            "known_value": np.expand_dims(bcl_u[:,0], axis = 1),
            "penalty": bc_left_penalty
        },
        # "dirichlet_2": {
        #     "coord": bcl_u_pts_1,
        #     "known_value": np.expand_dims(bcl_u_1[:,0], axis = 1),
        #     "penalty": bc_left_penalty
        # },
        # "dirichlet_2": {
        #     "coord": bcl_u_pts_2,
