...
class EnergyModel:
    # ---------------------------------------------------------------------------------------------------------------------------
    def __init__(self, energy, dim, E=None, nu=None, vm = None, Ec = None, l = None):
        self.type = energy
        self.dim = dim
    ...
    def CE(self,uz,pz,x): #constitutive equation
        ...
        # p = pz.detach()
        # x_load = torch.where((x[:,0]**2+x[:,1]**2) < (cf.a**2))[0]
        # p = torch.zeros(*pz.size())
        # p[x_load,:] = cf.ph*torch.sqrt(1-((x[x_load,0].unsqueeze(1))**2+(x[x_load,1].unsqueeze(1))**2)/cf.a**2)
        # p = p.detach()#.cpu()#.numpy()
        eta = cf.eta_i*torch.exp((np.log(cf.eta_i)+9.67)*((1+pz/cf.pi)**(0.67)-1))
        tau = eta/cf.Es_v
        beta = tau/(1+cf.Re)
        s_pz = pz+self.vm*beta*dpzdx
        s_uz = self.Ec/self.l*(uz+self.vm*tau*duzdx)
