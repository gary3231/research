...
import dem_hyperelasticity.two_object_contact.config as cf
...
class EnergyModel:
    ...
    def NBC(self, s, x, i):
        ...
        if i == 0:
            # l = self.normalize(x)
            return  torch.cat((x[:,0].unsqueeze(1)*sxx+(x[:,1].unsqueeze(1)-cf.origin[1])*sxy, x[:,0].unsqueeze(1)*syx+(x[:,1].unsqueeze(1)-cf.origin[1])*syy),1)
        if i == 1:
            # l = self.normalize(x)
            return  torch.cat((x[:,0].unsqueeze(1)*sxx+(x[:,1].unsqueeze(1)-cf.origin[1])*sxy, x[:,0].unsqueeze(1)*syx+(x[:,1].unsqueeze(1)-cf.origin[1])*syy),1)