...
def write_arr2DVTU(filename, x_space, y_space, z_space, U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises):
    x_flat = x_space.flatten()
    y_flat = y_space.flatten()
    z_flat = x_flat*0.0
    ux = U[0].flatten()
    uy = U[1].flatten()
    # E = E.flatten()
    ...
    # E12 = E12.flatten()
    # E22 = E22.flatten()
    assert len(x_flat) == len(y_flat) == len(z_flat) == len(ux) == len(uy), "Input arrays must have the same length"
    pointsToVTK(filename, x_flat, y_flat, z_flat, data={"ux": ux, "uy": uy,"S11": s11, "S12": s12, "S22": s22})
    return
# pointsToVTK(path, x_flat, y_flat, z_flat, data={"ux": ux,"uy": uy,"pxx": pxx,"pyy": pyy,"pxy": pxy})
# --------------------------------------------------------------------------------
# purpose: doing something in post processing for visualization in 3D
# --------------------------------------------------------------------------------
