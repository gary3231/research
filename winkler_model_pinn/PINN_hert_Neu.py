...
import os,sys
sys.path.append("")
from dem_hyperelasticity.winkler_model_pinn import define_structure_vis as des_v
from dem_hyperelasticity.winkler_model_pinn import define_structure_sub as des_s
from dem_hyperelasticity.MultiLayerNet_PINN import *
from dem_hyperelasticity.winkler_model_pinn import PINNmodel_neu as md
from dem_hyperelasticity import Utility as util
from dem_hyperelasticity.winkler_model_pinn import config as cf
from dem_hyperelasticity.IntegrationLoss import *
from dem_hyperelasticity.winkler_model_pinn.PINNmodel_neu import *
from dem_hyperelasticity.winkler_model_pinn import pin_alg as pin
from dem_hyperelasticity.winkler_model_pinn import substrate as sub
import numpy as np
import time
import torch
import math
...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim):
        ...
        self.model.load_state_dict(torch.load("winkler_De0_final.pt"))
        self.model.eval()
    ...
    def train_model(self, shape, dxdydz, set_dom_vis, set_dom_sub, LHD, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                # loss = 10*ss+15*b_loss+40*neu_loss+160*lkkt#+10**(-7)*g_loss#+1000*lkkt_un
                loss = 0.00001*ss+1*b_loss+40*neu_loss+0*lkkt
                # loss = 0.001*ss+0.1*b_loss+40*neu_loss+0*lkkt  #this one111
                # loss = 1*self.loss_squared_sum_2(uz,uz_2)+1*self.loss_squared_sum_2(pz,pz_2)+0.1*lkkt
                # if t >= 0:
                #     loss+=+0.05*b_loss#+0*lkkt
                # if t >= 200:
                #     loss+=0.05*b_loss+ss
            ...
            loss = closure()
            # with open('loss_his_De0_ss10.csv','a') as fd:
            #     fd.write(str(loss.item()) + '\n')
            if loss<loss_pre:
                time.sleep(0.02)
                # torch.save(self.model.state_dict(),'winkler_De0_ss10_final.pt')
                time.sleep(0.02)
            #     break
            # else:
                loss_pre = loss
...
if __name__ == '__main__':
    ...
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat = md.EnergyModel('neohookean', 2, cf.E, cf.nu, cf.vm, cf.beta, cf.Ec, cf.l, cf.tau)
    ...
    # np.save("winkler_uz_DE0_test.npy", U)
    # np.save("winkler_pz_DE0_test.npy", P)
