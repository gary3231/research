...
from dem_hyperelasticity.hertzian_contact_square.config import *
...
def setup_domain():
    ...
    bcl_u_pts_idx_2 = np.where(dom[:, 1] == Height)
    
    bcl_u_pts = dom[bcl_u_pts_idx, :][0]
    bcl_u_pts_2 = dom[bcl_u_pts_idx_2, :][0]   #get first position of dirichlet BC
    ...
    bcr_t_pts_idx_2 = np.where((dom[:, 1] == Height))
    bcr_t_pts_idx_3 = np.where((dom[:, 0] == Length))
    ...
    bcr_t_2 = np.ones(np.shape(bcr_t_pts_2)) * [0, -0.1]
