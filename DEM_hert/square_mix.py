...
import os,sys
sys.path.append("")
from dem_hyperelasticity.DEM_hert import define_structure as des
from dem_hyperelasticity.MultiLayerNet import *
from dem_hyperelasticity.DEM_hert import EnergyModel_mix as md
from dem_hyperelasticity import Utility as util
from dem_hyperelasticity.DEM_hert import config as cf
from dem_hyperelasticity.IntegrationLoss import *
from dem_hyperelasticity.DEM_hert.EnergyModel_mix import *
import numpy as np
import time
import torch
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                loss = internal2 - torch.sum(external2)+50*ss+1*lkkt_t+1000*lkkt+5000*boundary_loss+1*neu_loss   #scale problem?
                optimizer.zero_grad()
                loss.backward()
                print('Iter: %d Loss: %.6e Energy: %.6e   lkkt: %.6e ss: %.6eTime: %.3e'
                      % (t + 1, loss.item(), energy_loss.item(),lkkt.item(),ss.item() ,time.time() - it_time))
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------------------------
    start_time = time.time()
    shape = [cf.Nx, cf.Ny+20]
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 1500
    cf.filename_out = "dem_hyperelasticity/DEM_hert/output/MDEM_fb__domain_change"
