...
from dem_hyperelasticity.DEM_contact import EnergyModel as md
...
from dem_hyperelasticity.DEM_contact.EnergyModel import *
import numpy as np
import time
import torch
...
class DeepEnergyMethod:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                external2 = torch.zeros(len(neuBC_coordinates)-1)
                for i, vali in enumerate(neuBC_coordinates):
                    ...
                    if i ==1:
                        ...
                        pn[:,1] = p
                        fext = torch.bmm((neu_u_pred + neuBC_coordinates[i]).unsqueeze(1), pn.unsqueeze(2))
                        # kkt_f = self.intLoss.lossExternalEnergy(fext, dx=dxdydz[1])
                        # f_t = torch.bmm((neu_u_pred[:,0] + neuBC_coordinates[i][:,0]).unsqueeze(2), p.unsqueeze(3))
                        kkt_t = torch.tensor(0)#self.intLoss.lossExternalEnergy(fext, dx=dxdydz[0])

                        lkkt_u = self.loss_squared_sum_3((1-torch.sign(g))*g/2, 0)
                        kkt_f = self.intLoss.lossExternalEnergy(fext, dx=dxdydz[0])
                        ...
                        # lkkt_t = self.loss_squared_sum_2(kkt_t, 0)
                        lkkt_un = self.loss_squared_sum_3(kkt_f*g, 0)
                    # else:
                    #     fext = torch.bmm((neu_u_pred + neuBC_coordinates[i]).unsqueeze(1), neuBC_values[i].unsqueeze(2))
                ...
                boundary_loss = bc_u_crit[0][0]
                loss = internal2 - torch.sum(external2) + boundary_loss+100*lkkt_u-0.1*kkt_t+10*lkkt_un-kkt_f
                optimizer.zero_grad()
                loss.backward()
                print('Iter: %d Loss: %.6e Energy: %.6e lkkt_t: %.6e lkkt_u: %.6e lkkt_un: %.6e kkt_f: %.6eTime: %.3e'
                      % (t + 1, loss.item(), energy_loss.item(), kkt_t.item(),lkkt_u.item(),lkkt_un.item(),kkt_f.item() ,time.time() - it_time))
    ...
    def evaluate_model(self, x, y, z):
        ...
        Pxy=sigmaxy.reshape(Ny, Nx,1).detach().cpu().numpy()
        ...
        Eyy = (np.abs((sigmayy.detach().cpu().numpy()+0.1)/(-1-epsilon))).reshape(Ny, Nx,1)
        Exy = (np.abs(sigmaxy.detach().cpu().numpy()/(0-epsilon))).reshape(Ny, Nx,1)
...
if __name__ == '__main__':
    ...
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 40
    cf.filename_out = "dem_hyperelasticity/DEM_contact/output/2"
