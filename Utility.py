...
# purpose: doing something in post processing for visualization in 3D
# --------------------------------------------------------------------------------
def write_vtk_v2(filename, x_space, y_space, z_space, U,P,E):
    xx, yy, zz = np.meshgrid(x_space, y_space, z_space)
    gridToVTK(filename, xx, yy, zz, pointData={"displacement": U, "Pressure":P,"Error":E})
    # gridToVTK(filename, xx, yy, zz, pointData={"displacement": U, "P11":P[0],"P12":P[1],"P21":P[2],"P22":P[3],"P11_u":E[0],"P12_u":E[1],"P21_u":E[2],"P22_u":E[3]})
    # gridToVTK(filename, xx, yy, zz, pointData={"displacement": U})

def write_arr2DVTU(path, x, y, u, p, E):
    x_flat = x.flatten()
    y_flat = y.flatten()
    z_flat = x_flat*0.0
    ux = u[0].flatten()
