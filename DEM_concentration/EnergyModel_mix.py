...
class EnergyModel:
    ...
    def NeoHookean2D(self,u, x):
        ...
        epsxx_u = duxdxy[:,0].unsqueeze(1)
        epsxy_u = (0.5*duxdxy[:,1]+0.5*duydxy[:,0]).unsqueeze(1)
        epsyx_u = epsxy_u
        epsyy_u = duydxy[:,1].unsqueeze(1)

        treps = epsxx_u+epsyy_u

        sigmaxx = self.lam*treps+2*self.mu*epsxx_u
        sigmaxy = 2*self.mu*epsxy_u
        sigmayx = 2*self.mu*epsyx_u
        sigmayy = self.lam*treps+2*self.mu*epsyy_u
        ...
        strainEnergy_u = (sigmaxx*epsxx_u+sigmayy*epsyy_u+2*sigmaxy*epsxy_u)

        # epsxx = ((1+self.lam/2/self.mu)*sxx-self.lam/2/self.mu*syy)/2/(self.lam+self.mu)
        # epsxy = sxy/(2*self.mu)
        # epsyx = epsxy
        # epsyy = (syy-sxx)/2/self.mu+epsxx

        # strainEnergy = (sxx*epsxx+syy*epsyy+2*sxy*epsxy)
        return (0.5*strainEnergy_u)
    ...
    def CE(self,u,x): #constitutive equation
        ...
        return torch.cat((sigmaxx,sigmayy,sigmaxy),1)
